...
public class cexuchinaSpider extends DeepCrawler {
	...
	public cexuchinaSpider(String crawlPath) {
		super(crawlPath);
		// TODO Auto-generated constructor stub
		
		regexRule.addRule("http://seq.cn/article.*");
	...
	public Links visitAndGetNextLinks(Page page) {
		...
		if(url.contains("article")){
			...
			String src = srcExactor.getSrc("\"来自：(.*)\"",doc.toString());
			
		
			String picUrl = doc.select("td#article_content>p>a>img").attr("src");

			Elements newsContentElment = doc.select("td#article_content");
			...
			String title = doc.select("div.h.hm>h1").text();
...
//			
			String hitTimes = doc.select("em#_viewnum").text();
			System.out.println("hittimessss"+hitTimes);
			...
			if (time != null && time.length() >= 5 &&newsContent!= null &&newsContent.length()>=10) {
				...
				if (jdbcTemplate != null) {
					int updates = jdbcTemplate
							.update("insert into huashengDemo (ts,text,src,newsUrl,imageurl,grabbedTime,title,readedTimes) value(?,?,?,?,?,?,?,?)",
									time, newsContent, src, page.getUrl(), picUrl,
									grabbedTime, title,hitTimes);
					if (updates == 1) {
...
//			
	...
	public static void main(String[] args){
		...
		cexuchinaSpider crawler = new cexuchinaSpider(
				"/Users/y/Desktop/testWebcollecter/data/cexuchinaSpider");
		crawler.setThreads(5);
		
//		crawler.addSeed("http://seq.cn/portal.php?mod=list&catid=1");
		crawler.addSeed("http://seq.cn/portal.php?mod=list&catid=16");
		String pageUrl = "http://seq.cn/portal.php?mod=list&catid=1&page=";
		for (int i = 1; i < 10; i++) {
